...
管理端照片列表 SHALL provide sorting controls that let users order photos by creation time or shooting time in ascending or descending order. Shooting-time ordering MUST use each photo's effective photo time: `spec.dateTimeOriginal` when present, otherwise `metadata.creationTimestamp`.
...
- **THEN** 列表按有效照片时间降序排列，其中有效照片时间为 `spec.dateTimeOriginal` 或缺失 EXIF 时的 `metadata.creationTimestamp`

#### Scenario: Switch to shooting time ascending
- **WHEN** 用户在排序下拉中选择「拍摄时间（旧→新）」
- **THEN** 列表按有效照片时间升序排列，其中有效照片时间为 `spec.dateTimeOriginal` 或缺失 EXIF 时的 `metadata.creationTimestamp`
...
`GET /apis/console.api.photo.halo.run/v1alpha1/photos` SHALL accept a `sort` query parameter to control ordering, using the format `field,(asc|desc)`. Supported fields are `spec.dateTimeOriginal` and `metadata.creationTimestamp`. Sorting by `spec.dateTimeOriginal` MUST use each photo's effective photo time: `spec.dateTimeOriginal` when present, otherwise `metadata.creationTimestamp`.
