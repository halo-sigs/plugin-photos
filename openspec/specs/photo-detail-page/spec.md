...
The detail route SHALL expose the following model attributes to the `photo` template:

- `photo`: the current `PhotoVo`
- `neighbors`: the list of `PhotoVo` in the sliding window, in sort order, including the current photo
- `prev`: the `PhotoVo` immediately before the current photo in sort order, or absent/null when the current photo is the first
- `next`: the `PhotoVo` immediately after the current photo in sort order, or absent/null when the current photo is the last
- `position`: the 1-based index of the current photo within the filtered context
- `total`: the total count of photos in the filtered context
...
- `title`: the page title string (from `base.title` setting or the photo's `spec.displayName`)
...
- **THEN** the template can access `photo`, `neighbors`, `prev`, `next`, `position`, `total`, `group`, `page`, `size`, `backUrl`, `title`, `_templateId`, and `photoUrl` from the rendering context
